...
class TokenService {
  ...
  static verifyAccessToken(token: string): TokenPayload | null {
    try {
      const decoded = jwt.verify(token, process.env.USER_ACCESS_TOKEN_SECRET as string) as TokenPayload;
  ...
  static verifyRefreshToken(token: string): TokenPayload | null {
    try {
      const decoded = jwt.verify(token, process.env.USER_REFRESH_TOKEN_SECRET as string) as TokenPayload;
